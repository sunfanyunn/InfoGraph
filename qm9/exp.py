...
def train(epoch, use_unsup_loss):
    ...
    for data in tqdm(unsup_train_loader):
        data = data.to(device)
        optimizer.zero_grad()

        # sup_loss = F.mse_loss(model(data), data.y)
        unsup_loss = model.unsup(data)
        loss = unsup_loss
...
def logistic_classify(X_train, y_train, X_val, y_val, X_test, y_test):
    ...
    for it in range(1000):
        ...
        logits = log(X_train)
        loss = F.mse_loss(logits, y_train)
        ...
        log.eval()
        preds = log(X_val)
        val_error = (preds * std - y_val * std).abs().mean().item()  # MAE
        if val_error < best_val:

            best_val = val_error
            preds = log(X_test)
            test_error = (preds * std - y_test * std).abs().mean().item()  # MAE

            print(it, 'Val MAE', val_error, 'Test MAE', test_error)

    return test_error

def evaluate(train_loader, val_loader, test_loader):
    model.eval()
    with torch.no_grad():
        error = 0
        ...
        for data in train_loader:
            data = data.to(device)
            X_train.append(model.encoder(data)[0].cpu().numpy())
        ...
        for data in val_loader:
            data = data.to(device)
            X_val.append(model.encoder(data)[0].cpu().numpy())
        ...
        for data in test_loader:
            data = data.to(device)
            X_test.append(model.encoder(data)[0].cpu().numpy())
            y_test.append(data.y.cpu().numpy())
        X_test = np.concatenate(X_test, 0)
        y_test = np.concatenate(y_test, 0)

    return logistic_classify(X_train, y_train, X_val, y_val, X_test, y_test)
...
if __name__ == '__main__':
    ...
    for epoch in range(1, epochs):
        ...
        evaluate(train_loader, val_loader, test_loader)
